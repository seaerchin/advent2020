...
data Field = BYR Int | IYR Int | EYR Int | HGT Int Bool | ECL String | CID | PID String | HCL String 
    deriving (Show, Eq)
...
validate (HCL (x: xs)) = x == '#' && isValidDigit xs || isValidLetter xs && length xs == 6
...
mkField (key, value) 
    ...
    | key == "hgt" = 
        ...
        in if kind == "cm" then HGT (read height :: Int) True else HGT (read height :: Int) False 
    | key == "ecl" = ECL value 
    | key == "pid" = PID value 
    | key == "hcl" = HCL value
    | otherwise = CID
...
validatePassport m = all validate m && ((length m == 7 && CID `notElem` m) || (length m == 8))
